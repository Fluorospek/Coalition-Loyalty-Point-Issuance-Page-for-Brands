<script>
    ...
    async function handleSubmit(event) {
        ...
        try {
            const response = await axios.post('https://coalition-loyalty-point-issuance-page.onrender.com/brand/setup', {
                brandName,
                description,
            }, {
                headers: {
    ...
    <div class="py-6 px-4 mx-auto max-w-screen-md">
        ...
        {#if authenticated}
            <form on:submit={handleSubmit} class="space-y-4">
                <div>
                    <label for="brandName" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                        Brand Name
                    </label>
                    <input
                        type="text"
                ...
                <div class="sm:col-span-2">
                    <label for="brandDescription" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">
                        Brand Description
                    </label>
                    <textarea
                        id="brandDescription"
                ...
                <div>
                    <label for="otherDetails" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                        Other Details
                    </label>
                    <input
                        type="text"
